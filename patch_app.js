...
// remove the simulator switch
const switchRegex = /\{\/\* SIMULATOR SWITCH \[\s\S\]\*\/\}[\s\S]*?(<\/select>[\s\S]*?<\/div>[\s\S]*?<\/div>)/;
// Actually I will just find the exact text and replace it with empty string
// Let me first write out the content so I can inspect.
